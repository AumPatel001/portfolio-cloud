...
__export(schema_exports, {
  ...
  insertPageVisitSchema: () => insertPageVisitSchema,
  insertProjectInquirySchema: () => insertProjectInquirySchema,
...
var pageVisits = pgTable("page_visits", {
  id: serial("id").primaryKey(),
...
var insertPageVisitSchema = createInsertSchema(pageVisits).pick({
  page: true,
  visitorIp: true,
  userAgent: true,
  referer: true
...
var DatabaseStorage = class {
  ...
  async markContactSubmissionAsResponded(id) {
    const [updated] = await db.update(contactSubmissions).set({ responded: true }).where(eq(contactSubmissions.id, id)).returning();
    return updated;
  }
  // Analytics methods
  async recordPageVisit(visit) {
  ...
  async getProjectInquiries() {
    return await db.select().from(projectInquiries).orderBy(projectInquiries.createdAt);
  }
  async updateProjectInquiryStatus(id, status) {
    const [updated] = await db.update(projectInquiries).set({ status }).where(eq(projectInquiries.id, id)).returning();
    return updated;
  }
};
...
// server/routes.ts
var trackPageVisit = async (req, res, next) => {
  try {
...
async function registerRoutes(app2) {
  ...
  app2.get("/api/analytics", async (req, res) => {
    try {
      const pageVisits2 = await storage.getPageVisits();
      const pageViews = pageVisits2.length;
      const uniqueVisitors = new Set(pageVisits2.map((visit) => visit.visitorIp)).size;
      const pageViewsByPath = pageVisits2.reduce((acc, visit) => {
